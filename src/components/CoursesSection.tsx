    ...
    <section id="courses" className="pt-16 pb-2 bg-background relative overflow-hidden">
      ...
      <div className="container relative z-10">
        ...
        <div className="min-h-[300px]">
          <div
            className={`${activeTab === "UG" || activeTab === "PG" ? "flex flex-wrap justify-center gap-6" : "grid gap-6"} ${activeTab === "UG" || activeTab === "PG" ? "" : "grid-cols-1 md:grid-cols-2 lg:grid-cols-4"}`}
          >
            {currentCourses.map((course) => {
              ...
              return (
                <div
                  key={course.name}
                  className={`flex flex-col h-full bg-gradient-to-br from-white to-gray-50 hover:from-accent/5 hover:to-primary/5 backdrop-blur-sm border border-gray-200 hover:border-accent/60 rounded-3xl p-7 transition-all duration-300 overflow-hidden relative shadow-sm hover:shadow-xl hover:-translate-y-1 ${activeTab === "UG" || activeTab === "PG" ? "w-full lg:w-[calc(25%-12px)]" : ""}`}
                >
                  ...
                  <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-accent to-primary text-white flex items-center justify-center mb-4 hover:scale-110 hover:shadow-lg transition-all duration-300 shadow-sm flex-shrink-0">
                    <Icon className="w-7 h-7" />
                  </div>

                  <h3 className="text-lg font-bold text-foreground mb-3 hover:text-accent transition-colors duration-300 min-h-[56px] line-clamp-2" dangerouslySetInnerHTML={{ __html: course.name }}>
                  </h3>

                  <div className="flex-grow flex flex-col">
                    <div className="bg-accent/5 rounded-lg p-3 mb-4 min-h-[80px] flex-grow">
                      <p className="text-xs text-foreground mb-1 font-semibold">Eligibility</p>
                  ...
                  <div className="mt-auto flex-shrink-0">
                    <button
                      ...
                      className="w-full bg-gradient-to-r from-accent to-primary text-white hover:from-accent/90 hover:to-primary/90 rounded-xl px-4 py-2.5 text-sm font-semibold transition-all duration-300 flex items-center justify-center gap-2 shadow-sm hover:shadow-md hover:scale-105 min-h-[44px]"
                    >
                      Know More
