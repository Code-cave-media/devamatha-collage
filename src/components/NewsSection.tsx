...
const NewsSection = () => {
  ...
  useEffect(() => {
    // Sort news by ID in descending order and take first 4
    const sortedNews = [...fullNewsData].sort((a, b) => b.id - a.id).slice(0, 4);
    
    const newsWithIcons = sortedNews.map(item => ({
      title: item.title,
      date: item.date,
      description: item.description,
      icon: iconMap[item.category] || Megaphone,
      category: item.category
    }));
    setAnnouncements(newsWithIcons);
  }, []);
            ...
            >
              <div className="flex items-start gap-4">
                <motion.div 
                  ...
                  className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${getIconColor(announcement.category)}`}
                >
